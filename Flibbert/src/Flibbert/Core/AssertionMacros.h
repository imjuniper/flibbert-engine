...
struct AssertionInfo
{
	...
	constexpr AssertionInfo(const char* InCustomMessage, const char* InCondition, const char* InFile,
	                        uint32_t InLine)
	    : CustomMessage(InCustomMessage), Condition(InCondition), File(InFile), Line(InLine)
	{}
};
...
void EnsureImpl(const bool useCoreLogger, const AssertionInfo& info, Args&&... args)
{
	...
	#define FBT_ENSURE_IMPL_MSG(condition, core, fmt, ...)                                                         \
		(!!(condition) || [] {                                                                                 \
			...
			::Flibbert::Assert::Private::EnsureImpl(core, s_info __VA_OPT__(, ) __VA_ARGS__);              \
			FBT_DEBUG_BREAK();                                                                             \
			return false;                                                                                  \
		}())
