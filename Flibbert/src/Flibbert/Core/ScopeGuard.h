...
template <typename Callback, typename = std::enable_if_t<std::is_nothrow_destructible_v<Callback>>>
class ScopeGuard;
...
struct SyntaxHelper
{
	...
	ScopeGuard<Callback>
	operator+(Callback&& callback) noexcept(std::is_nothrow_constructible_v<Callback, Callback&&>)
	{
...
private:
	explicit ScopeGuard(Callback&& callback) noexcept(std::is_nothrow_constructible_v<Callback, Callback&&>)
	    : m_Callback(std::forward<Callback>(callback))
	{}

	friend ScopeGuard<Callback> SyntaxHelper::operator+
