...
void OpenGLVertexArray::AddVertexBuffer(std::shared_ptr<VertexBuffer> vertexBuffer)
{
	...
	for (const auto& element : layout) {
		...
		case ShaderDataType::Float4: {
			...
			glVertexArrayAttribFormat(m_RendererID, m_VertexAttributeIndex, componentCount, GL_FLOAT,
			                          element.Normalized ? GL_TRUE : GL_FALSE, offset);
		...
		case ShaderDataType::Int4: {
			glEnableVertexArrayAttrib(m_RendererID, m_VertexAttributeIndex);
			glVertexArrayAttribFormat(m_RendererID, m_VertexAttributeIndex, componentCount, GL_INT,
			                          element.Normalized ? GL_TRUE : GL_FALSE, offset);
			glVertexArrayAttribBinding(m_RendererID, m_VertexAttributeIndex, vboIndex);
			m_VertexAttributeIndex++;
			break;
		...
		case ShaderDataType::Bool: {
			glEnableVertexArrayAttrib(m_RendererID, m_VertexAttributeIndex);
			glVertexArrayAttribFormat(m_RendererID, m_VertexAttributeIndex, componentCount, GL_BOOL,
			                          element.Normalized ? GL_TRUE : GL_FALSE, offset);
			glVertexArrayAttribBinding(m_RendererID, m_VertexAttributeIndex, vboIndex);
			m_VertexAttributeIndex++;
			break;
