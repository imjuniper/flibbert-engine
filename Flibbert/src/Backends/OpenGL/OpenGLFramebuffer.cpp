...
void OpenGLFramebuffer::Invalidate()
{
	...
	if (m_RendererID == 0) {
		...
		glNamedFramebufferRenderbuffer(m_RendererID, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, m_DepthBufferID);
	}

	// Set their size
	glTextureStorage2D(m_ColorBufferID, 1, GL_RGB8, m_Specification.Width, m_Specification.Height);
	glNamedRenderbufferStorage(m_DepthBufferID, GL_DEPTH24_STENCIL8, m_Specification.Width, m_Specification.Height);

	FBT_CORE_ENSURE_MSG(glCheckNamedFramebufferStatus(m_RendererID, GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE,
