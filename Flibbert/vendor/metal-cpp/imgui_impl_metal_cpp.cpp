...
#else
#error "Did you download metal-cpp headers ? Cannot find the metal-cpp headers !"
#endif
// clang-format on
...
struct MetalContext {
	...
	[[nodiscard]] NS::SharedPtr<MTL::RenderPipelineState>
	renderPipelineStateForFramebufferDescriptor(
	    const FramebufferDescriptor* framebufferDescriptor) const;
	void synchronizedBufferCache(
	    std::function<void(std::vector<std::shared_ptr<MetalBuffer>>&)>&& fn);

	MTL::Device* device;
	NS::SharedPtr<MTL::DepthStencilState> depthStencilState;
	const FramebufferDescriptor* framebufferDescriptor;
	std::unordered_map<const FramebufferDescriptor*, NS::SharedPtr<MTL::RenderPipelineState>>
...
// ------------------------------------------------------------------------------------------------
static inline double GetMachAbsoluteTimeInSeconds()
{
	return static_cast<double>(static_cast<double>(clock_gettime_nsec_np(CLOCK_UPTIME_RAW)) /
	                           1e9);
}
...
bool ImGui_ImplMetal_CreateFontsTexture(MTL::Device* device)
{
	...
	// memory access bandwidth. However, using a shader designed for 1-channel texture would
	// make it less obvious to use the ImTextureID facility to render users own textures. You
	// can make that change in your implementation.
	...
	bd->SharedMetalContext->fontTexture->replaceRegion(
	    MTL::Region(0, 0, (NS::UInteger)width, (NS::UInteger)height), 0, pixels,
	    (NS::UInteger)width * 4);
...
static void
ImGui_ImplMetal_SetupRenderState(ImDrawData* drawData, MTL::CommandBuffer* commandBuffer,
                                 MTL::RenderCommandEncoder* commandEncoder,
                                 NS::SharedPtr<MTL::RenderPipelineState> renderPipelineState,
                                 const MetalBuffer* vertexBuffer, size_t vertexBufferOffset)
{
	...
	// draw_data->DisplayPos+data_data->DisplaySize (bottom right). DisplayMin is typically
...
IMGUI_IMPL_API void ImGui_ImplMetal_NewFrame(MTL::RenderPassDescriptor* renderPassDescriptor)
{
	...
	static std::unordered_map<MTL::RenderPassDescriptor*,
	                          std::unique_ptr<FramebufferDescriptor>>
	    fbdm;
	if (fbdm.find(renderPassDescriptor) == fbdm.end()) {
		fbdm[renderPassDescriptor] =
		    std::make_unique<FramebufferDescriptor>(renderPassDescriptor);
                                                   ...
                                                   MTL::RenderCommandEncoder* commandEncoder)
{
	...
	// Try to retrieve a render pipeline state that is compatible with the framebuffer config
	// for this frame The hit rate for this cache should be very near 100%.
	NS::SharedPtr<MTL::RenderPipelineState> renderPipelineState;
	...
	if (renderPipelineStateIt != ctx->renderPipelineStateCache.end()) {
		...
		auto [insertedIt, succeeded] = ctx->renderPipelineStateCache.insert(
		    {ctx->framebufferDescriptor, renderPipelineState});
		IM_ASSERT(succeeded &&
		          "Could not insert a new render pipeline state in the cache !");
	}
	...
	ImGui_ImplMetal_SetupRenderState(drawData, commandBuffer, commandEncoder,
	                                 renderPipelineState, vertexBuffer.get(), 0);
			...
			if (pCmd->UserCallback) {
				...
				if (pCmd->UserCallback == ImDrawCallback_ResetRenderState) {
					ImGui_ImplMetal_SetupRenderState(
					    drawData, commandBuffer, commandEncoder,
					    renderPipelineState, vertexBuffer.get(),
					    vertexBufferOffset);
				} else {
				...
				// Clamp to viewport as setScissorRect() won't accept values that
				// are off bounds
				if (clip_min.x < 0.0f) {
					clip_min.x = 0.0f;
				}
				...
				if (ImTextureID tex_id = pCmd->GetTexID()) {
					commandEncoder->setFragmentTexture((MTL::Texture*)(tex_id),
					                                   0);
				}
				...
				commandEncoder->drawIndexedPrimitives(
				    MTL::PrimitiveTypeTriangle, pCmd->ElemCount,
	...
	commandBuffer->addCompletedHandler(^(MTL::CommandBuffer*) {
	  if (bd) {
		  bd->SharedMetalContext->synchronizedBufferCache(
		      [&](std::vector<std::shared_ptr<MetalBuffer>>& bufferCache) {
...
// ------------------------------------------------------------------------------------------------
ImGui_ImplMetal_Data::ImGui_ImplMetal_Data() : SharedMetalContext(nullptr) {}

ImGui_ImplMetal_Data::~ImGui_ImplMetal_Data()
{
	if (SharedMetalContext) {
		IM_DELETE(SharedMetalContext);
	}
}

MetalBuffer::MetalBuffer(NS::SharedPtr<MTL::Buffer> buffer)
    : buffer(buffer), lastReuseTime(GetMachAbsoluteTimeInSeconds())
{
}
...
bool MetalBuffer::operator<(const MetalBuffer& other) const
{
...
bool FramebufferDescriptor::operator==(const FramebufferDescriptor& other) const
{
	return (sampleCount == other.sampleCount && colorPixelFormat == other.colorPixelFormat &&
	        ...
	        stencilPixelFormat == other.stencilPixelFormat);
}

std::size_t std::hash<FramebufferDescriptor>::operator()(const FramebufferDescriptor& fbd) const
{
...
MetalContext::MetalContext(MTL::Device* device)
    : device(device), depthStencilState(), framebufferDescriptor(nullptr),
      renderPipelineStateCache(), fontTexture(), _bufferCache(),
...
{
	synchronizedBufferCache(
	    [&](std::vector<std::shared_ptr<MetalBuffer>>& bufferCache) { bufferCache.clear(); });
	renderPipelineStateCache.clear();
	device->release();
}

std::shared_ptr<MetalBuffer> MetalContext::dequeueReusableBuffer(NS::UInteger length)
{
	...
	synchronizedBufferCache([&](std::vector<std::shared_ptr<MetalBuffer>>& bufferCache) {
		...
		for (auto it = bufferCache.begin(); it != bufferCache.end(); ++it) {
			if ((*it)->buffer->length() >= length &&
			    (bestCandidateIt == bufferCache.end() ||
			     (*bestCandidateIt)->lastReuseTime > (*it)->lastReuseTime)) {
				bestCandidateIt = it;
			}
		}
		...
		auto backing =
		    NS::TransferPtr(device->newBuffer(length, MTL::ResourceStorageModeShared));
		bestCandidate = std::make_unique<MetalBuffer>(backing);
	});
	return bestCandidate;
}

NS::SharedPtr<MTL::RenderPipelineState> MetalContext::renderPipelineStateForFramebufferDescriptor(
    const FramebufferDescriptor* framebufferDescriptor) const
{
	// clang-format off
	...
	auto vertexFunction =
	    NS::TransferPtr(library->newFunction(NS_STRING_FROM_CSTRING("vertex_main")));
	IM_ASSERT(vertexFunction && "Failed to find metal vertex shader function");

	auto fragmentFunction =
	    NS::TransferPtr(library->newFunction(NS_STRING_FROM_CSTRING("fragment_main")));
...
void MetalContext::synchronizedBufferCache(
    std::function<void(std::vector<std::shared_ptr<MetalBuffer>>&)>&& fn)
{
