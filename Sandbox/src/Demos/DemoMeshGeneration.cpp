...
void DemoMeshGeneration::OnUpdate(float ts)
{
	...
	const PerFrameUniformData buffer{m_Camera->GetViewMatrix(), m_Camera->GetProjectionMatrix(),
...
void DemoMeshGeneration::OnImGuiRender()
{
	...
	if (ImGui::CollapsingHeader("Mesh Settings", ImGuiTreeNodeFlags_DefaultOpen)) {
		ImGui::Indent(16.0f);
		ImGui::InputScalar("Side Length", ImGuiDataType_U16, &m_SideLength, &sideLengthStep,
		                   &sideLengthStepFast);
	...
	if (ImGui::CollapsingHeader("Noise Settings", ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		ImGui::SliderFloat("Gradient Rotation", &m_MeshGenUniformData.GradientRotation, -180.0f, 180.0f);
		ImGui::SliderInt("Octaves", &m_MeshGenUniformData.Octaves, 1, 32);
		if (ImGui::CollapsingHeader("Octave Settings")) {
			ImGui::Indent(16.0f);
			ImGui::SliderFloat("Rotation", &m_MeshGenUniformData.NoiseRotation, -180.0f, 180.0f);
			ImGui::InputFloat2("Angular Variance", glm::value_ptr(m_MeshGenUniformData.AngularVariance));
			ImGui::SliderFloat("Initial Amplitude", &m_MeshGenUniformData.InitialAmplitude, -0.01f, 2.0f);
			ImGui::SliderFloat("Amplitude Decay", &m_MeshGenUniformData.AmplitudeDecay, -0.01f, 1.0f);
			ImGui::SliderFloat("Lacunarity", &m_MeshGenUniformData.Lacunarity, -0.01f, 3.0f);
			...
			ImGui::SliderFloat("Height Scale", &m_MeshGenUniformData.TerrainHeight, 0.f, 500.0f);
			ImGui::Unindent(16.0f);
		}
		ImGui::Unindent(16.0f);
	}
	...
	if (ImGui::CollapsingHeader("Material Settings", ImGuiTreeNodeFlags_DefaultOpen)) {
		ImGui::Indent(16.0f);
		ImGui::SliderFloat("Slope Damping", &m_MeshGenUniformData.SlopeDamping, 0.0f, 1.0f);
		ImGui::SliderFloat2("Slope Threshold", glm::value_ptr(m_MeshGenUniformData.SlopeRange), 0.0f, 1.0f);
		ImGui::ColorEdit4("Low Slope Color", glm::value_ptr(m_MeshGenUniformData.LowSlopeColor));
		ImGui::ColorEdit4("High Slope Color", glm::value_ptr(m_MeshGenUniformData.HighSlopeColor));
		ImGui::SliderFloat("Normal Strength", &m_MeshGenUniformData.NormalStrength, 0.0f, 1.0f);
		ImGui::Unindent(16.0f);
	}
	...
	if (ImGui::CollapsingHeader("Light Settings", ImGuiTreeNodeFlags_DefaultOpen)) {
		ImGui::Indent(16.0f);
		ImGui::SliderFloat3("Light Direction", glm::value_ptr(m_MeshGenUniformData.LightDirection), -1.0f,
		                    1.0f);
		ImGui::ColorEdit3("Ambient Light", glm::value_ptr(m_MeshGenUniformData.AmbientLight));
		ImGui::Unindent(16.0f);
	}
}
